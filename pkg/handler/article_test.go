...
func TestGetTagsCaseSuccess(t *testing.T) {
	...
	rec := httptest.NewRecorder()
	c := e.NewContext(req, rec)
	assert.NoError(t, h.MediaTags(c))
	if assert.Equal(t, http.StatusOK, rec.Code) {
