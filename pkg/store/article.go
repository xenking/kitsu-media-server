...
func (as *ArticleStore) GetBySlug(s string) (*model.Article, error) {
	...
	err := as.db.Where(&model.Article{Content: model.Content{Slug: s}}).Preload("Favorites").Preload("Tags").Preload("Author").Find(&m).Error
	if err != nil {
...
func (as *ArticleStore) GetUserArticleBySlug(userID uint, slug string) (*model.Article, error) {
	...
	err := as.db.Where(&model.Article{Content: model.Content{Slug: slug, AuthorID: userID}}).Find(&m).Error
	if err != nil {
		if gorm.IsRecordNotFoundError(err) {
			return nil, nil
		}
...
func (as *ArticleStore) ListByAuthor(username string, offset, limit int) ([]model.Article, int, error) {
	...
	as.db.Where(&model.Article{Content: model.Content{AuthorID: u.ID}}).
		Preload("Favorites").
		Preload("Tags").
		Preload("Author").
		Offset(offset).
		...
		Order("created_at desc").
		Find(&articles)
	as.db.Where(&model.Article{Content: model.Content{AuthorID: u.ID}}).Model(&model.Article{}).Count(&count)
...
func (as *ArticleStore) ListByWhoFavorited(username string, offset, limit int) ([]model.Article, int, error) {
	...
	as.db.Model(&u).
		...
		Limit(limit).
		Order("created_at desc").
		Association("ArticleFavorites").
		Find(&articles)

	count = as.db.Model(&u).Association("ArticleFavorites").Count()
...
func (as *ArticleStore) GetCommentsBySlug(slug string) ([]model.Comment, error) {
	var m model.Article
	err := as.db.Where(&model.Article{Content: model.Content{Slug: slug}}).Preload("Comments").Preload("Comments.User").First(&m).Error
